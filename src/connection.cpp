    ...
    if (config.get() != nullptr) {
        ...
        refreshHushdState(connection, [=] () {
            ...
            if (Settings::getInstance()->useEmbedded()) {
                if (tryEhushdStart) {
                    if(iszdeex) {
                        this->showInformation(QObject::tr("Starting embedded zdeexd"));
                    } else {
                    ...
                    if (config->hushDaemon) {
                        explanation = QString() % QObject::tr("You have hushd set to start as a daemon, which can cause problems "
                            ...
                            "daemon=1");
                        if (iszdeex) {
                            explanation = QString() % QObject::tr("You have zdeexd set to start as a daemon, which can cause problems "
                            "with SilentZdeeX\n\n."
                        ...
                        if(iszdeex) {
                            explanation = QString() % QObject::tr("Couldn't start the embedded zdeexd.\n\n"
                            "Please try restarting.\n\nIf you previously started hushd with custom arguments, you might need to  reset ZDEEX.conf.\n\n"
                            "If all else fails, please run zdeexd manually.") %
                            (ehushd ? QObject::tr("The process returned") + ":\n\n" % ehushd->errorString() : QString(""));
                        }
                    }
                ...
                if(iszdeex) {
                    main->logger->write("Not using embedded and couldn't connect to zdeexd");
                    QString explanation = QString() % QObject::tr("Couldn't connect to zdeexd configured in ZDEEX.conf.\n\n"
                                          "Not starting embedded zdeexd because --no-embedded was passed");
                }
                this->showError(explanation);
            }
        });
...
void ConnectionLoader::createHushConf() {
    ...
    if (!file.open(QIODevice::ReadWrite | QIODevice::Truncate)) {
        QString explanation = QString() % (iszdeex ? QObject::tr("Could not create ZDEEX.conf.") : QObject::tr("Could not create HUSH3.conf.") );
    ...
    if(iszdeex) {
        out << "# Autogenerated by Hush SilentZdeeX " << APP_VERSION << " https://zdeex.com\n";
    } else {
...
bool ConnectionLoader::startEmbeddedHushd() {
    ...
    // params for ZDEEX are handled below
#else
    auto hushdProgram = appPath.absoluteFilePath("hushd");
    if (iszdeex) {
        hushdProgram = appPath.absoluteFilePath("zdeexd");
    }
#endif
        ...
        [=] (QNetworkReply* reply, const QJsonValue &res) {
            ...
            if (err == QNetworkReply::NetworkError::ConnectionRefusedError) {
                ...
                if(iszdeex) {
                    explanation = QString() %
                        QObject::tr("Authentication failed. The username / password you specified was "
                        "not accepted by zdeexd. Try changing it in the Edit->Settings menu");
                }
                ...
                if(iszdeex) {
                    this->showInformation(QObject::tr("Your zdeexd is starting up. Please wait."), status);
                } else {
...
QString ConnectionLoader::hushConfWritableLocation() {
    ...
    if(iszdeex) {
        confLocation = QDir(QStandardPaths::writableLocation(QStandardPaths::HomeLocation)).filePath(".hush/ZDEEX/ZDEEX.conf");
    }
#elif defined(Q_OS_DARWIN)
    ...
    if(iszdeex) {
        confLocation = QDir(QStandardPaths::writableLocation(QStandardPaths::HomeLocation)).filePath("Library/Application Support/Hush/ZDEEX/ZDEEX.conf");
    }
#else
    auto confLocation = QDir(QStandardPaths::writableLocation(QStandardPaths::AppDataLocation)).filePath("../../Hush/HUSH3/HUSH3.conf");
    if(iszdeex) {
        confLocation = QDir(QStandardPaths::writableLocation(QStandardPaths::AppDataLocation)).filePath("../../Hush/ZDEEX/ZDEEX.conf");
    }
#endif
...
bool ConnectionLoader::verifyParams() {
    ...
    if( QFile( QDir("/Applications").filePath("silentzdeex.app/Contents/MacOS/sapling-output.params") ).exists() && QFile( QDir("/Applications").filePath("./silentzdeex.app/Contents/MacOS/sapling-spend.params") ).exists() ) {
        qDebug() << "Found params in /Applications/silentzdeex.app/Contents/MacOS";
        return true;
    }

    // this is to support SDX on mac inside a DMG
    if( QFile( QDir("./").filePath("silentzdeex.app/Contents/MacOS/sapling-output.params") ).exists() && QFile( QDir("./").filePath("./silentzdeex.app/Contents/MacOS/sapling-spend.params") ).exists() ) {
        qDebug() << "Found params in ./silentzdeex.app/Contents/MacOS";
        return true;
    }
 ...
 * Try to automatically detect a HUSH3/HUSH3.conf file
   or ZDEEX/ZDEEX.conf in the correct location and load parameters
 */

...
void RPC::getInfoThenRefresh(bool force) {
    ...
    conn->doRPC(makePayload(method), [=] (const QJsonValue& reply) {
        ...
        // only look at local hashrate for zdeex
        if(iszdeex) {
            QString method = "getlocalsolps";
        ...
        */
        ...
        conn->doRPCIgnoreError(makePayload("getmininginfo"), [=](const QJsonValue& reply) {
            ...
            if(iszdeex) {
                ...
                // The rest of this callback is for ZDEEX
                return;
            }
        ...
        conn->doRPCIgnoreError(makePayload("getblockchaininfo"), [=](const QJsonValue& reply) {
            ...
            // No price data for zdeex for now
            if (!iszdeex) {
...
void RPC::shutdownHushd() {
    ...
    if(iszdeex) {
        connD.status->setText(QObject::tr("Please enhance your calm and wait for SilentZdeeX to exit"));
        connD.statusDetail->setText(QObject::tr("Waiting for zdeexd to exit, y'all"));
    } else {
